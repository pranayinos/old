...
$newsRecordSet = $student->getdata('batch.batch_id,batch.batch_code,course.course_name,course.course_section_name',"course,batch","course.course_code = batch.batch_course_code","course_name ASC","");// Course
 ...
 //update query in student_profile
    if(isset($_POST['updateNew_bt'])) // If the submit button was clicked
    {
			...
			//image upload   			   
			//echo $select=mysql_query("select file from student where id='$roll'");
 			//echo $row=mysql_fetch_array($select);
 			//echo $file1=$row[file];
			 
			  //$student = new Main(); // create a new news object
   			//$roll=$_GET[id];
 			// $field1 = $student->getdata('*',"student","id='$roll'","", "");// set newsRecordSet to a  
			  // $record = $field1->getNextRecord(); 
    		 //echo  $record['file'];
			 //end of the code of image 
			 // INSRET THE CODE OF IMAGE UPLOAD 
...
imagedestroy($tmp1);
	//header("location:postadd.php?action=1&title=$title&msg=Submitted the Data ");
			 ...
			 //End the code image upload  
				...
				header("location:?page=student_profile&id=$roll&msg=$msg");	
...
<?php }?>
 ...
 <span class="span-sub-heading">Upload User Photo</span>
        ...
        <div class="label-field-pair">
        <?php /*?> <input type="file" name="file" value="upload/<?php echo $row2[file]?>">
         <img src="upload/<?php echo $row2[file]?>" height="100" width="120" style="float:left" /><?php */?>
         
          <label for="student_image_file">Upload User Photo (5 KB max)</label>
          <div class="text-input-bg"><input id="file" name="file" size="12" type="file" value="upload<?=$record['file']?>"></div>
