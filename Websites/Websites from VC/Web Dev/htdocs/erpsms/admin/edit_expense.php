...
$expencecatSet = $expence1->getdata('*',"finance_category","income=0 and active =1","finance_cat_name ASC", "");
    ...
    if(isset($_POST['update'])) // If the submit button was clicked
    {
      // batch entry 
		   ...
		   if($expence1->updateData($post,"fin_transaction","tr_id='$id'"))
        {
				...
				header("location:?page=edit_expense&msg=$msg");			
		}
		else
		{
			$msg="Error :Entry not done, Please check Values";
			header("location:?page=edit_expense&error=$msg");
		}		
	}
		?>
