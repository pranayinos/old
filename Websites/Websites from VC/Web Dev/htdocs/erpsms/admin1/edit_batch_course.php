...
//add the page 28 aug 2012  run  CODE 
	...
	//$course=$_GET[course];
	 ...
	 //$batch_code=$_GET[batch_code];
 	 $newsRecordSet = $batch->getdata('*',"batch","batch_id='$id'","batch_id DESC", "");
 ...
 if(isset($_POST['btsubmit'])) // If the submit button was clicked
    {
      // manage course update 
			 $batch_id=$_POST['id'];  
			 $post['batch_code'] =$_POST['batch_code'];
		     $post['batch_start_date'] =$_POST['batch_start_date'];
			 $post['batch_end_date'] =$_POST['batch_end_date'];
			  
			   if($batch->updateData($post,"batch","batch_id='$id'"))
				 {
				 $msg="News updated Succesfully";
				header("location:?page=edit_batch_course&batch_id=$id&msg=$msg");			
				}
				else
				{
					$msg="Error :  Entry not done, Please check Values";
					header("location:?page=edit_batch_course&batch_id=$id&error=$msg");
				}		
	}
...
<!--------- Start head ---------------->
<!-------------Insert the 15 sep  code of validation--------------------------------------------->
  <script>
...
<form name="formID" id="formID" action="#" class="edit_batch"  method="post"><div style="margin:0;padding:0;display:inline">
<input id="batch_code" name="batch_code" type="hidden" value="<?=$record['batch_id']?>">
</div>

      <div class="label-field-pair">
        <div class="label-container">Name</div>
   <div class="input-container"><input id="batch_code" name="batch_code" class="validate[required]  text-input" size="30" type="text" value="<?=$record['batch_code']?>"></div>
      </div>
