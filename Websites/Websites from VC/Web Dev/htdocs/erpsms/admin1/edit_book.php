 ...
 if(isset($_POST['commit'])) // If the submit button was clicked
    {
      ...
      // news entry 
			   $post['book_number'] =$_POST['book_number'];
			   $post['book_title'] =$_POST['book_title'];
			   $post['author'] =$_POST['author'];
			   $post['custom_tag'] =$_POST['custom_tag'];
				$post['no_of_copy'] =$_POST['no_of_copy'];
				
			   
			   
		if($add_books->updateData($post,"add_books","book_id='$id'"))
        {
        
		 		$msg="Book Record Updated Succesfully";
				header("location:?page=edit_book&msg=$msg");			
		}
		else
		{
			$msg="Error :  Entry not done, Please check Values";
			header("location:?page=edit_book&error=$msg");
		}		
	}
...
<div id="page-yield">
  ...
  <div class="search_form">
    ...
    <form name="formID" id="formID" action="" class="edit_book" id="edit_book_30" method="post"><div style="margin:0;padding:0;display:inline"><input name="_method" type="hidden" value="put"><input name="authenticity_token" type="hidden" value="Q82vux645k6qzovSWYhmFGB4wWYAjv9l0xSjcltafEc="></div>
       <div class="msg" ><?php echo $_GET[msg]?></div>
 <div class="error" ><?php echo $_GET[error]?></div>
      <div class="label-field-pair">
        <label for="book_number">Book Number <span class="necessary-field">*</span></label>
        <div class="text-input-bg">
          <input id="book_book_number" name="book_number" size="30" type="text" value="<?=$record['book_number'];?> " class="validate[required]  text-input">
        </div>
      </div>
      <div class="label-field-pair">
        <label for="title">Title <span class="necessary-field">*</span></label>
        <div class="text-input-bg">
          <input id="book_title" name="book_title" size="30" type="text" value="<?=$record['book_title'];?>" class="validate[required]  text-input">
        </div>
      </div>
      ...
      <div class="label-field-pair">
        ...
        <div class="text-input-bg">
          <input id="book_author" name="author" size="30" type="text" value="<?=$record['author'];?>" class="validate[required]  text-input">
        </div>
      </div>
     ...
     <div class="tag">
      ...
      <div class="label-field-pair">
        <label for="title">Custom Tags </label>
            <div class="text-input-bg">
              <input id="tag_list" name="custom_tag" type="text" class="validate[required]  text-input" value="<?=$record['custom_tag'];?>">
            </div>
          </div>
      ...
      <div class="label-field-pair">
        ...
        <div class="text-input-bg">
          <select id="book_status" name="status">
