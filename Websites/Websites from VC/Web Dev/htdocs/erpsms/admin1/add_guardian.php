...
$newsRecordSet = $gaurdian->getdata('*',"gaurdian","stud_id='$s_id'","", "");
...
//INSERT CODE IMPLEMENT 
	 ...
	 {
	    $post['stud_id'] =$_POST['id'];
			    $post['guard_first_name'] =$_POST['parent_detail_first_name'];
			    $post['guard_last_name'] =$_POST['parent_detail_last_name'];
			    $post['guard_rel'] =$_POST['parent_detail_relation'];
				$post['guard_dob'] =date(("Y-m-d"), strtotime($_POST['parent_detail_dob']));		
			    //$cols['guard_dob'] =$_POST['parent_detail_dob'];
				$post['guard_edu'] =$_POST['parent_detail_education'];
				$post['guard_occu'] =$_POST['parent_detail_occupation'];
				$post['guard_income'] =$_POST['parent_detail_income'];
			    $post['guard_email'] =$_POST['parent_detail_email'];
			    $post['guard_off_add1'] =$_POST['parent_detail_office_address_line1'];
				$post['guard_off_add2'] =$_POST['parent_detail_office_address_line2'];
				$post['guard_city'] =$_POST['parent_detail_city'];
				$post['guard_state'] =$_POST['parent_detail_state'];
				$post['guard_country'] =$_POST['parent_detail_country_id'];
				$post['guard_off_ph1'] =$_POST['parent_detail_office_phone1'];
				$post['guard_off_ph2'] =$_POST['parent_detail_office_phone2'];
				$post['guard_mobile_no'] =$_POST['parent_detail_mobile_phone'];
				
				if($gaurdian->updateData($post,"gaurdian","stud_id='$s_id'"))
           {
				$msg="  Gaurdian Updated Succesfully";
				header("location:?page=add_guardian&msg=$msg");			
		   }
		   else
		   {
	 ...
	 else
	 {
      ...
      // employee payslip implemented by 8 nov 2012
				...
				$cols['guard_dob'] =date(("Y-m-d"), strtotime($_POST['parent_detail_dob']));		
			    //$cols['guard_dob'] =$_POST['parent_detail_dob'];
...
?>
        ...
        <div class="text_display"><?php echo $_GET['id']?></div>
      ...
      <div class="label-field-pair">
        ...
        <div class="text-input-bg"><input id="parent_detail_first_name" name="parent_detail_first_name" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_first_name'];?>"></div>
      </div>
      <div class="label-field-pair">
        <label for="parent_detail_name">Last Name</label>
        <div class="text-input-bg"><input id="parent_detail_last_name" name="parent_detail_last_name" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_last_name'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_relation">Relation<span class="necessary-field">*</span></label>
        <div class="text-input-bg"><input id="parent_detail_relation" name="parent_detail_relation" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_rel'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_dob">Date of Birth</label>
        <div class="text-input-bg">
          <input id="parent_detail_dob" name="parent_detail_dob" type="text" value="<?=$record['guard_dob'];?>"> <img alt="Calendar" class="calendar_date_select_popup_icon" onClick="new CalendarDateSelect( $(this).previous(), {year_range:[1952, 1992]} );" src="../images/calendar_date_select/calendar.gif" style="border:0px; cursor:pointer;">
        </div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_education">Education</label>
        <div class="text-input-bg"><input id="parent_detail_education" name="parent_detail_education" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_edu'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_occupation">Occupation</label>
        <div class="text-input-bg"><input id="parent_detail_occupation" name="parent_detail_occupation" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_occu'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_income">Income</label>
        <div class="text-input-bg"><input id="parent_detail_income" name="parent_detail_income" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_income'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_email">Email</label>
        <div class="text-input-bg"><input id="parent_detail_email" name="parent_detail_email" size="30" type="text" class="validate[required,custom[email]] text-input" value="<?=$record['guard_email'];?>"></div>
      </div>
      ...
      <div class="label-field-pair">
        <label for="master_student_city">Office Address Line 1</label>
          ...
          <div class="text-input-bg"><input id="parent_detail_office_address_line1" name="parent_detail_office_address_line1" size="30" type="text"  class="validate[required]  text-input" value="<?=$record['guard_off_add1'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="master_student_city">Office Address Line 2</label>
          
          <div class="text-input-bg"><input id="parent_detail_office_address_line2" name="parent_detail_office_address_line2" size="30" type="text"  class="validate[required]  text-input"value="<?=$record['guard_off_add2'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_city">City</label>
          <div class="text-input-bg"><input id="parent_detail_city" name="parent_detail_city" size="30" type="text"  class="validate[required]  text-input" value="<?=$record['guard_city'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_state">State</label>
          <div class="text-input-bg"><input id="parent_detail_state" name="parent_detail_state" size="30" type="text"  class="validate[required]  text-input" value="<?=$record['guard_state'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_country">Country</label>
          <div class="text-input-bg"> <select id="parent_detail_country_id" name="parent_detail_country_id" value="<?=$record['guard_country'];?>"><option value="1">Afghanistan</option>
...
<option value="195">Zimbabwe</option></select> </div>
      ...
      <div class="label-field-pair">
        ...
        <div class="text-input-bg"><input id="parent_detail_office_phone1" name="parent_detail_office_phone1" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_off_ph1'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_office_phone2">Office phone 2</label>
        <div class="text-input-bg"><input id="parent_detail_office_phone2" name="parent_detail_office_phone2" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_off_ph2'];?>"></div>
      </div>

      <div class="label-field-pair">
        <label for="parent_detail_mob_phone">Mobile Phone no.</label>
        <div class="text-input-bg"><input id="parent_detail_mobile_phone" name="parent_detail_mobile_phone" size="30" type="text" class="validate[required]  text-input" value="<?=$record['guard_mobile_no'];?>"></div>
      </div>
