<?php  
 $subject = new Main(); // create a new news object
   $id=$_GET[subjectid];
	$subjectRecordSet = $subject->getdata('*',"subject","subject_id=$id","subject_id DESC","50");
	
//<!---------- 31 Aug edit and update header not work  subject  Code of update query  ---------------->

	if(isset($_POST['btsubmit'])) // If the submit button was clicked
    {
      // manage subject update 
			 $subject_id=$_POST['id'];
		     $post['subject_name'] =$_POST['subject_name'];
			 $post['subject_max_weekly_classes'] =$_POST['subject_max_weekly_classes'];
			  
			   if($subject->updateData($post,"subject","subject_id='$id'"))
				 {
				 $msg="Subject Name Updated Succesfully";
				header("location:?page=subject&subject_id=$id&msg=$msg");			
				}
				else
				{
					$msg="Error :  Entry not done, Please check Values";
				header("location:?page=edit_subject&subject_id=$id&error=$msg");
				}		
	}
...
<div class="box">
<div id="MB_window" style="width: 500px; height: 368px; left: 382px; overflow: visible; "><div id="MB_frame" style=""><div id="MB_header"><div id="MB_caption">Edit Subject</div></div><div id="MB_content" style=""><div id="modal-box" style="">
...
<input id="subject_id" name="subject_id" size="30" type="hidden" class="MB_focusable" value="<?=$record['subject_id']?>">
  
     <div id="form-errors"></div>

  <div class="label-field-pair">
    <div class="label-container"><label>Name</label></div>
    <div class="input-container"> <input id="subject_name" name="subject_name" class="validate[required]  text-input" size="30" type="text" value="<?=$record['subject_name']?>"></div>
  </div>

    <div class="label-field-pair">
    <div class="label-container">  <label>Code</label></div>
    <div class="input-container"><input id="subject_code" name="subject_code" disabled="true" size="30" type="text" class="MB_focusable" value="<?=$record['subject_code']?>"></div>
  </div>

    <div class="label-field-pair">
    <div class="label-container"><label>Max weekly classes</label></div>
    <div class="input-container"> <input id="subject_max_weekly_classes" name="subject_max_weekly_classes" size="30" type="text" class="MB_focusable" value="<?=$record['subject_max_weekly_classes']?>"></div>
    </div>
  ...
  <div class="label-field-pair">
   ...
   <input class="submit_button" id="btsubmit" name="btsubmit" type="submit" title="submit form" value="► Save">
...
<img align="absmiddle" alt="Loader" border="0" id="loader" src="./subject_files/loader.gif" style="display: none; ">
  ...
  <?php /*?><select id="batch_id" name="batch_id" onChange="showsubject(this.value)" >
