...
if($q!="")
{
$newsRecord = $student->getdata('id,std_adm_no,std_first_name,std_middle_name,std_last_name,std_batch_code',"student","std_first_name LIKE '$q%'","", "");
}
...
if($n!="")
{
$newsRecord = $news->getdata('*',"news","news_title LIKE '$n%'","", "");

//$newsRecord = $news->getdata('*',"news","","news_id ASC", "30"); 
...
<table id="listing">
  <tbody><tr class="tr-head">
    ...
    <td class="col-4">Comments</td>
    <td class="col-5">Posted</td>
  </tr>';
   $i=0;   
   while( ($record = $newsRecord->getNextRecord()) !== false )
    { 
	$i++;  
 	  echo '<tr class="tr-odd ">
	   <td class="col-1">'.$record[news_id].'</td>
      <td class="col-2"><a href="?page=news-view&id='.$record[news_id].'">'.$record[news_title].'</a></td>
     
	  <td class="col-3">'.$record[news_postedby].'</td>
      <td class="col-4">'.$record[news_desc].'</td>
      <td class="col-5">'.$record[news_date].'</td>
...
echo '<option value="'.$record[subject_name].'"> '.$record[subject_name].'  </option>';
 }
echo '</select> ';
}
...
if($dues_fee_coll_id1!="")
{
	...
	$newsRecord1 = $emp->getdata('*',"emp","emp_dept_code='$dues_fee_coll_id1'","", "");
	...
	echo '  <table align="center" width="100%" cellpadding="1" cellspacing="1" id="listing">
	 ...
	 while( ($strecord = $newsRecord1->getNextRecord()) !== false )
		...
		echo'<tr class="tr-odd">
			  ...
			  <td class="col-1">'.$strecord['emp_first_name'].'</td>
			   <td class="col-1">'.$strecord['emp_dept_code'].''.
			   '&nbsp;'."<FONT COLOR='RED'>[CURRENTYLY ASSIGNED SUBJECT]</FONT>".
			   ''.'<a href="?page=hr_department_payslip&id='.$strecord[emp_no].'">Delete</a>'.'</td>
			  </tr>';
			 }
...
echo '</tbody></table>';
//department code 
//START  THE DEPARMENT CODE 
	    if($dues_fee_coll_id1!="")
{

		echo '<select id="fees_dues_coll" name="fees_dues_coll" onChange=\'show_fee_dues_student1(this.value)\' />
       <option value="">Select Department </option>';
	   $newsRecord = $emp_dept->getdata('*',"emp_dept","","", "");
	while(($record = $newsRecord->getNextRecord()) !== false )
{
	//echo  $fee_pay_id=$stfeepayrecord['fee_pay_id'];
   echo $a= $record[emp_dept];
echo '<option value="'.$record[emp_dept].'"> '.$record[dept_desc].'  </option>';
 }
echo '</select> ';
}	
	   //END THE DEPARTMENT CODE
//end of recall deparment 
...
$emp=new Main();
$batchRecordSet1 = $emp->getdata('*',"emp","emp_dept_code='$dues_fee_student_batch2'","","");// batch
...
echo '  <select id="fees_dues_coll_date" name="fees_dues_coll_date" onChange=\'show_fee_dues_student(this.value)\' />
	...
	while(($record4 = $batchRecordSet1->getNextRecord()) !== false )
{
$emp_no=$record4['emp_no'];

$newsRecord = $emp_payslip->getdata('*',"emp_payslip","emp_no='$emp_no'","", "");
$record = $newsRecord->getNextRecord();
	//echo  $fee_pay_id=$stfeepayrecord['fee_pay_id'];
  
echo '<option value="'.$record['sal_date'].'"> '.$record['sal_date'].'  </option>';
 }
echo '</select> ';
}
...
{
	$newsRecord1 = $emp_payslip->getdata('*',"emp_payslip","sal_date='$dues_fee_coll_id2'","", "");
	echo '  <table align="center" width="100%" cellpadding="1" cellspacing="1" id="listing">
		<tbody><tr class="tr-head">
		  <td>Sl-No.</td>
		  <td>Employee</td>
	 ...
	 while( ($strecord = $newsRecord1->getNextRecord()) !== false )
		...
		echo'<tr class="tr-odd">
			  <td class="col-5">'.$i.'</td>
			  <td class="col-1">'.$record3['emp_first_name'].''.$record3['emp_mid_name'].''.$record3['emp_last_name'].'</td>
			   <td class="col-1">'.$strecord['emp_no'].'</td>
			    <td class="col-1">'.$strecord['basic_sal'].'</td>
			   <td class="col-1">'.$strecord['status'].'</td>
			   <td class="col-1">'.'<a href="?page=emp_final_payslip&id='.$strecord[emp_no].'">VIEW</a>'.'</td>
			  </tr>';
...
echo '<table id="listing" align="center" width="100%" cellpadding="1" cellspacing="1">
 ...
 while( ($record = $newsRecord->getNextRecord()) !== false )
	...
	<tr class="tr-odd">
	  <td class="col-1">'.$i.'</td>
	  <td class="col-3">'.$record[std_reg_no].'</td>
	  <td class="col-4"><a href="?page=student_registration_profile&id='.$record[stud_id].'">'.$record[std_first_name].' '.$record[std_middle_name].' '.$record[std_last_name].'</a></td>
	  <td class="col-4">'.$courserow[course_name].' '.$courserow[course_section_name] .'-'.$courserow[batch_code].'</td>
	  <td class="col-7"><a href="?page=student_registration_profile&id='.$record[stud_id].'">View Profile</a></td>
	</tr>
   ';
    } 
  }
