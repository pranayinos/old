...
$newsRecord = $emp_payslip->getdata('*',"emp_payslip","payslip_id='$dues_fee_student_batch2'","", "");
	while(($record = $newsRecord->getNextRecord()) !== false )
{
	//echo  $fee_pay_id=$stfeepayrecord['fee_pay_id'];
   echo $a= $record[sal_date];
echo '<option value="'.$record[subject_batch_code].'"> '.$record[sal_date].'  </option>';
 }
echo '</select> ';
}
