...
$newsRecordSet1 = $student->getdata('*',"time_setting","ts_id='$ts_id'","","");
...
if(isset($_POST['commit'])) // If the submit button was clicked
    {
      // batch entry 
			   ...
			   $post['sl'] =$_POST['pr'];
			   $post['start_time'] =$_POST['start_time'];
			   $post['end_time'] =$_POST['end_time'];
			  
			   

   if($student->updateData($post,"time_setting","ts_id='$ts_id'"))
        {
				$msg="New Time Setting  Updated Succesfully";
				//header("location:?page=admission&msg=$msg");	
				header("location:?page=edit_class_time_table&msg=$msg");			
		}
		else
		{
			$msg="Error :  Entry not done, Please check Values";
			header("location:?page=edit_class_time_table&error=$msg");
		}		
...
<label>SL:</label>
<div class="text-input-bg">
<input id="pr" class="validate[required]  text-input" name="pr"  size="30" type="text" title="" value="<?=$record1['sl'];?>" class="validate[required]  text-input"/>
...
<div class="text-input-bg">
<input id="start_time" class="validate[required]  text-input" name="start_time"  size="30" type="text" title="" value="<?=$record1['start_time'];?>" class="validate[required]  text-input"/>
...
<div class="text-input-bg">
<input id="end_time" class="validate[required]  text-input" name="end_time"  size="30" type="text" title="" value="<?=$record1['end_time'];?>" class="validate[required]  text-input"/>

</div></div>
<div class="extender" align="left"> <input class="submit_button" name="commit" type="submit" value="► Save"><!-- Massage -->
