...
class Mysql
{
     ...
     *          - $table = <span id="IL_AD8" class="IL_AD">The table</span> you want <span id="IL_AD1" class="IL_AD">to return</span> information from.
     *          - $where = This allows you to pick with database records you want to return.
     *          - $orderby = This determinds <span id="IL_AD10" class="IL_AD">the return</span> order of the select.
...
//echo "SELECT $fields FROM $table" . $where . $orderby . $limit ;
        $recordSet =
            mysql_query(
                ...
                "Selecting $table - SELECT $fields FROM $table"  . $where . $orderby . $limit .
                    ...
                    //    <span id="IL_AD6" class="IL_AD">print this</span> string to the screen.
            );
        if (!$recordSet) // A quick <span id="IL_AD11" class="IL_AD">check</span> to see if the query failed. This is a backup to the previos die()
        {
    ...
    {
        if($where != '') $where = " WHERE $where";
      // echo "UPDATE $table SET $fields" .$where."";
        $query =
            mysql_query(
