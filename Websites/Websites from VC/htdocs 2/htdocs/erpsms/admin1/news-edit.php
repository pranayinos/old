...
$news = new Main();
$newsedit_id=$_GET[id];	
$field = $news->getdata('*',"news","news_id='$newsedit_id'", "",""); // News
...
$newsedit_id=$_GET[id];	
 ...
 if(isset($_POST['commit'])) // If the submit button was clicked
    {
      // news update 
	  		   $post['news_id']=$newsedit_id;
			   $post['news_title'] =$_POST['news_title'];
			   $post['news_desc'] =$_POST['news_desc'];
			   
			   // $news_title =$_POST['news_title'];
			   
			      if($news->updateData($post,"news","news_id='$newsedit_id'"))
				 {
				   $msg="News updated Succesfully";
				header("location:?page=news-view&id=$newsedit_id&msg=$msg");			
		}
		else
		{
  ...
  <form action="#" method="post"><div style="margin:0;padding:0;display:inline"><input name="authenticity_token" type="hidden" value="LJs/NJ/YW+9+c9m63t0T4PpHBHeoosdOtJiMijBKVfw="></div>
    ...
    <div id="add_news_form">
      <div class="news_label_field">
        ...
        <div id="news_title_text_field_bg"><input id="news_title" name="news_title" size="30" type="text" 
		value="<?=$record['news_title'] ?>"></div>
      </div>

      <div class="news_label_field">
        <label for="news_content">Content</label>
        <div id="news_content_text_field_bg">
        <textarea id="news__content_editor" name="news_desc" cols="75" rows="16" style="border:none"><?=$record['news_desc'] ?></textarea>
</div>
      </div>
    </div>
